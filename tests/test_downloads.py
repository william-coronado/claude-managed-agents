...
class TestDownloadSessionOutputs:
    ...
    def test_falls_back_when_duplicate_basename_and_size_are_ambiguous(self, tmp_path):
        """Two files with the same basename AND the same byte size can't be told apart -
        guessing which is which would risk silently swapping their content between
        destinations, so both fall back to their own logged content instead."""
        client = _make_client()
        client.beta.sessions.events.list.return_value = [
            _make_write_event("/mnt/session/outputs/todo/config.py", "X = 1"),
            _make_write_event("/mnt/session/outputs/utils/config.py", "X = 1"),
        ]
        _wire_files_api(client, [
            _make_file("file_1", "config.py", len(b"X = 1")),
            _make_file("file_2", "config.py", len(b"X = 1")),
        ])

        count = self._call(client, "sess-12", tmp_path)

        assert count == 2
        assert (tmp_path / "todo" / "config.py").read_text() == "X = 1"
        assert (tmp_path / "utils" / "config.py").read_text() == "X = 1"
        client.beta.files.download.assert_not_called()

    def test_retry_ignores_unrelated_scoped_files(self, tmp_path):
        """The retry loop must track whether the *requested* basenames have landed, not
        just whether the total scoped file count looks big enough - an unrelated file
        scoped to the session (e.g. a mounted input) could otherwise satisfy the count
        before the actual output file has finished indexing."""
        client = _make_client()
        client.beta.sessions.events.list.return_value = [
            _make_write_event("/mnt/session/outputs/result.py", "print('ok')"),
        ]
        unrelated_then_match = [
            [_make_file("file_unrelated", "input.csv", 999)],
            [
                _make_file("file_unrelated", "input.csv", 999),
                _make_file("file_1", "result.py", len(b"print('ok')")),
            ],
        ]
        client.beta.files.list.side_effect = unrelated_then_match
        ...
        with patch("src.downloads.time.sleep") as mock_sleep:
            count = self._call(client, "sess-13", tmp_path)
    ...
    def test_falls_back_to_logged_content_when_files_list_raises(self, tmp_path):
        """A Files API outage must not prevent recovering files at all - it should
        degrade to the same logged-content fallback used for an unmatched file."""
        client = _make_client()
        client.beta.sessions.events.list.return_value = [
            _make_write_event("/mnt/session/outputs/result.py", "print('ok')"),
        ]
        ...
        count = self._call(client, "sess-14", tmp_path)
    ...
    def test_falls_back_to_logged_content_when_download_raises(self, tmp_path):
        """A download failure for one matched file must not abort the whole run -
        it should fall back to that file's logged content."""
        client = _make_client()
        client.beta.sessions.events.list.return_value = [
            _make_write_event("/mnt/session/outputs/result.py", "print('ok')"),
        ]
        client.beta.files.list.return_value = [_make_file("file_1", "result.py", len(b"print('ok')"))]
        client.beta.files.download.side_effect = RuntimeError("download failed")

        count = self._call(client, "sess-15", tmp_path)
